<template>
  <view class="index">
    ...
    <LiveTime :TDS="data.TDS" :Temperature="data.Temperature" :LightSwitch="data.LightSwitch" />
    <ControlPanel :TDS="data.TDS" :Temperature="data.Temperature" :LightSwitch="data.LightSwitch" :Pump="data.Pump"
      :HeatSwitch="data.HeatSwitch" @heatUp="(type) => heatUp(type)" />
  </view>
...
const queryDeviceProperty = async () => {
  try {
    ...
    if (response.data.code === 0) {
      ...
      data.LightSwitch = response.data.data.find(item => item.identifier === 'LightSwitch')?.value == 'true';
      data.Pump = response.data.data.find(item => item.identifier === 'Pump')?.value == 'true';
      data.HeatSwitch = response.data.data.find(item => item.identifier === 'HeatSwitch')?.value == 'true';
