  ...
  <view class="container">
    ...
    <nut-grid :column-num="3" reverse>
      <nut-grid-item :border="false" text="水温(°C)">
        <TrendArrow
          :number="Number(props.data.Temperature)"
          :status="isOutOfRange('Temperature')"
        />
      </nut-grid-item>
      <nut-grid-item :border="false" text="水质(ppm)">
        <TrendArrow
          :number="Number(props.data.TDS)"
          :status="isOutOfRange('TDS')"
        />
      </nut-grid-item>
      <nut-grid-item :border="false" text="光照(Lux)">
        <TrendArrow
          :number="Number(props.data.Light)"
          :status="isOutOfRange('Light')"
        />
      </nut-grid-item>
    </nut-grid>
  </view>
...
const isOutOfRange = (key) => {
  ...
  if (key === "Light") {
    ...
    if (val < min) return "high";
    else if (val > max) return "low";
    else return "normal";
  }
};
...
const processQueue = async () => {
  ...
  while (changeQueue.length > 0) {
    const command = changeQueue.shift();
    console.log("Processing command:", command);

    await change(command);
    await new Promise((resolve) => setTimeout(resolve, 3000)); // 每条命令之间间隔 2 秒
  }
...
watch(
  () => [
    ...
    props.data.Light,
    props.data.LightSwitch,
    props.data.HeatSwitch,
    props.data.Pump,
    props.control,
  ],
  ([
    ...
    if (
      !newPump &&
      (parseFloat(newTDS) < tdsRange.min || parseFloat(newTDS) > tdsRange.max)
    ) {
      scheduleChange({ Pump: true }); // 启动循环泵
    }
    if (
      newPump &&
      parseFloat(newTDS) > tdsRange.min &&
      parseFloat(newTDS) < tdsRange.max
    ) {
      scheduleChange({ Pump: false }); // 启动循环泵
    }
    if (!newLightSwitch && parseFloat(newLight) > lightRange.max) {
